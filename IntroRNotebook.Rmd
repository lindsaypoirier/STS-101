...
sts_majors <- 999
stats_majors <- 999
other<- 999 #Remember how Bowker and Star would refer to this as a residual category.
...
```{r}
birth_months <- c(999, 999, 999, 999, 999)
first_letter_name <- c("FILL","FILL","FILL","FILL","FILL")
time_on_phone <- c(999, 999, 999, 999, NA)
```

We can measure the length of a vector by calling the function *length*.
```{r}
length(birth_months)
```
...
We can perform operations on vectors - finding their sum or their average, for example. However, we may get an error if we have any empty values in the dataset. To avoid this, we need to communicate to R to remove NA values. 
```{r}
sum(time_on_phone)
mean(time_on_phone)
